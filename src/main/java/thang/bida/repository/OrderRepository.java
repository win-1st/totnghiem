...
public interface OrderRepository extends JpaRepository<Order, Long> {
  ...
  /**
   * Kiểm tra xem bàn có order active không (OPEN hoặc WAITING_PAYMENT)
   */
  @Query("SELECT COUNT(o) > 0 FROM Order o " +
      "WHERE o.table.id = :tableId " +
      "AND o.status IN ('OPEN', 'WAITING_PAYMENT')")
  boolean existsActiveOrderByTableId(@Param("tableId") Long tableId);

  /**
   * Đếm số lượng order của bàn (không bao gồm PAID)
   */
  ...
  /**
   * Lấy tất cả order của bàn (không bao gồm PAID)
   */
